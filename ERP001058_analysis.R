...
SRX = c()
for (item in metadata$relation) {
  split_result <- strsplit(item, "=")
  second_part <- split_result[[1]][2]
  SRX = c(SRX, second_part)
}
rownames(metadata) = SRX
sra_metadata = read.csv("/Users/rusiq/Downloads/ERP001058_run.csv")
sra_metadata = sra_metadata[,c("Run", "Experiment")]
dim(sra_metadata)
View(merged_data)
rownames(sra_metadata) = sra_metadata$Experiment
merged_data <- merge(metadata, sra_metadata, by = "row.names", all = TRUE)
merged_data$"relation.2" = NULL
merged_data = na.omit(merged_data)

control_vec <- merged_data[merged_data$source_name_ch1 == "Adjacent lung normal cells", "Run"]

#READ EXPRESSION
expr_df = read.csv("/Users/rusiq/Desktop/IGR/ERP001058_quant.tsv", sep = "\t")
...
#LOAD LOH
loh <- readBAFExtractOutput(path="/Users/rusiq/Desktop/IGR/ERP001058_snp/",
...
saveRDS(final.objects, "ERP001058_finals.rds")
...
write.csv(test_df, "ERP001058_deletions.csv")
