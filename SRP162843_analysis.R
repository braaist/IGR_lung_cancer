...
sra_metadata = read.csv("/Users/rusiq/Downloads/SRP162843_run.csv")
merged_data <- merge(metadata, sra_metadata, 
                     by.x = "row.names", by.y = "SampleName",
                     all = TRUE)

merged_data <- merged_data[-c(1, 2, 3, 4, 5, 6, 7, 8),]
control_vec <- merged_data[merged_data$source_name_ch1 == "adjacent lung", "Run"]

expr_df = read.csv("/Users/rusiq/Desktop/IGR/SRP162843_quant.tsv", sep = "\t")
...
loh <- readBAFExtractOutput(path="/Users/rusiq/Desktop/IGR/SRP162843_snp/",
...
saveRDS(final.objects, "SRP162843_finals.rds")
...
write.csv(test_df, "SRP162843_deletions.csv")
